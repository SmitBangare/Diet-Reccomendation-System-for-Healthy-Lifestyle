...
try:
    ...
    # Fallback in case the path is different when deployed
    try:
        dataset = pd.read_csv('./Data/dataset.csv', compression='gzip')
    except:
        # Another common path format
        dataset = pd.read_csv('Data/dataset.csv', compression='gzip')
...
# Main function to generate recommendations directly without API call
def generate_recommendations(nutrition_input, ingredients=[], params={'n_neighbors':5, 'return_distance':False}):
